...
import { deletefavMovie, selectedMovie } from '../actions/index';
...
class FavouriteMovies extends Component {
	...
	render() {
		...
		return (
			<div>
				<h2 class="mdl-card__title-text">Bookmarks</h2>
				<div>{this.mapMovies().length === 0 ? <div>there is no movie in the FavouriteMovies</div> : this.mapMovies()}</div>
			</div>
		);
	}
}
